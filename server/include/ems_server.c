...
static uint32_t data_send(ems_server_t * p_server,
                          const access_message_rx_t * p_message,
                          const ems_response_params_t * p_params)
{
    ...
    access_message_tx_t reply = {
        ...
        .p_buffer         = (const uint8_t *) &msg_pkt,
        .length           = EMS_RESPONSE_MESSAGE_LEN,
        .force_segmented  = p_server->settings.force_segmented,
        .transmic_size    = p_server->settings.transmic_size
    };
...
static void handle_get(access_model_handle_t model_handle, const access_message_rx_t * p_rx_msg, void * p_args)
{
    ...
    if(p_rx_msg->length == EMS_GET_COMMAND_MESSAGE_LEN)
    {
        ...
        {
            p_server->settings.p_callbacks->ems_cbs.get_cb(p_server, &p_rx_msg->meta_data, &in_data, &out_data);
            (void)data_send(p_server, p_rx_msg, &out_data);
        }
    }
}
